...
const ModalSeekCreditNotesProv = ({ setOpen, data }) => {
  ...
  const rows = dataCreditNotes.map((item) => ({
    ...
    mixedName: IntlMessages(Number(item.isMixed) === 1 ? 'option.yes' : 'option.no')
  }));
  ...
  const [table] = useState({
    ...
    columns: [
      ...
      { text: IntlMessages("page.creditNotesProv.check.isMixed"), dataField: "mixedName", headerStyle: { width: "10%" } },
      {
