...
const Admonitions = (props) => {
  const dispatch = useDispatch();

  const PRIVILEGE_CODE = "07.02.008";
  const screenControl = getPrivilegeData(PRIVILEGE_CODE);

  useEffect(() => {
    dispatch(onTitleEdit("menu.admonitions"))
    dispatch(onBreadcrumbEdit(`${adminRoot}/humanResources/process/admonitions`))
  }, [])
