...
const Biweeklys = (props) => {
  const dispatch = useDispatch();

  const PRIVILEGE_CODE = "07.01.009";
  const screenControl = getPrivilegeData(PRIVILEGE_CODE);

  useEffect(() => {
    dispatch(onTitleEdit("menu.biweeklys"))
    dispatch(onBreadcrumbEdit(`${adminRoot}/humanResources/settings/biweeklys`))
  }, [])
