...
export const useReasons = ({ setLoading }) => {
  ...
  const fnGetData = () => {
    setLoading(true);
    request.GET('hospital/settings/reasons', (resp) => {
  ...
  const fnOkDelete = () => {
    ...
    setLoading(true);
    request.DELETE(`hospital/settings/reasons/${idRecord}`, (resp) => {
      setLoading(false);
      setIdRecord(0);
      fnGetData();
    }, (err) => {
  ...
  const [table, setTable] = useState({
    title: IntlMessages("menu.hospitalManagement.reasonsAdmission"),
    columns: [
