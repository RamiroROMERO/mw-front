...
export const useProviderTypes = ({setLoading}) => {
  ...
  const providerTypesValid = {
    name: [(val) => val.length > 5, "msg.required.input.name"],
    idCtaCxp: [(val) => val !== "", "msg.required.select.idCtaCxp"],
  ...
  const fnSave = () => {
    ...
    if (formState.id > 0) {
      setLoading(true);
      request.PUT(`admin/providerTypes/${formState.id}`, formState, () => {
        fnClearInputs();
        fnGetData();
        setLoading(false);
      }, (err) => {
      ...
      setLoading(true);
      request.POST('admin/providerTypes', formState, () => {
        fnClearInputs();
        fnGetData();
        setLoading(false);
      }, (err) => {
  ...
  const fnDisableDocument = () => {
    ...
    if (formState.id && formState.id > 0) {
      setLoading(true);
      request.PUT(`admin/providerTypes/${formState.id}`, data, () => {
        fnGetData();
        fnClearInputs();
        setLoading(false);
      }, (err) => {
