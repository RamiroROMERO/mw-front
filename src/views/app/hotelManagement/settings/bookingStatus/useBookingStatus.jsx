...
export const useBookingStatus = ({ setLoading, screenControl }) => {
  ...
  const fnGetData = (page = currentPage, searchText = search) => {
    // setLoading(true);
    // request.GET(buildUrl('hotel/settings/bookingStatuses/paginate', { page, limit: pageSize, q: searchText }), (resp) => {
  ...
  const fnSaveDocument = () => {
    ...
    if (validInt(id) === 0) {
      ...
      setLoading(true);
      request.POST('hotel/settings/bookingStatuses', formState, () => {
        setLoading(false);
        fnGetData();
        fnClear();
      }, (err) => {
      ...
      setLoading(true);
      request.PUT(`hotel/settings/bookingStatuses/${id}`, formState, () => {
        setLoading(false);
        fnGetData();
        fnClear();
      }, (err) => {
  ...
  const fnDisableDocument = () => {
    ...
    if (formState.id && formState.id > 0) {
      setLoading(true);
      request.PUT(`hotel/settings/bookingStatuses/${formState.id}`, data, () => {
        fnGetData();
        fnClear();
        setLoading(false);
      }, (err) => {
