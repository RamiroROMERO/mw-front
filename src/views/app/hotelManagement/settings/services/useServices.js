...
export const useServices = ({ setLoading, screenControl }) => {
  ...
  const fnClear = () => {
    ...
    if (validInt(id) === 0) {
      ...
      setLoading(true);
      request.POST('hotel/settings/services', formState, () => {
        setLoading(false);
        fnGetData();
        fnClear();
      }, (err) => {
      ...
      setLoading(true);
      request.PUT(`hotel/settings/services/${id}`, formState, () => {
        setLoading(false);
        fnGetData();
        fnClear();
      }, (err) => {
  ...
  const fnDisableDocument = () => {
    ...
    if (formState.id && formState.id > 0) {
      setLoading(true);
      request.PUT(`hotel/settings/services/${formState.id}`, data, () => {
        fnGetData();
        fnClear();
        setLoading(false);
      }, (err) => {
