...
export const useBookingChannels = ({ setLoading, screenControl }) => {
  ...
  const fnSaveDocument = () => {
    ...
    if (validInt(id) === 0) {
      ...
      setLoading(true);
      request.POST('hotel/settings/bookingChannels', formState, () => {
        setLoading(false);
        fnGetData();
        fnClear();
      }, (err) => {
      ...
      setLoading(true);
      request.PUT(`hotel/settings/bookingChannels/${id}`, formState, () => {
        setLoading(false);
        fnGetData();
        fnClear();
      }, (err) => {
  ...
  const fnDisableDocument = () => {
    ...
    if (formState.id && formState.id > 0) {
      setLoading(true);
      request.PUT(`hotel/settings/bookingChannels/${formState.id}`, data, () => {
        fnGetData();
        fnClear();
        setLoading(false);
      }, (err) => {
