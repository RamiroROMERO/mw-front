...
export const useRoomLevels = ({ setLoading, screenControl }) => {
  ...
  const fnGetData = (page = currentPage, searchText = search) => {
    setLoading(true);
    request.GET(buildUrl('hotel/settings/roomLevels/paginate', { page, limit: pageSize, q: searchText }), (resp) => {
  ...
  const fnSaveDocument = () => {
    ...
    if (validInt(id) === 0) {
      ...
      setLoading(true);
      request.POST('hotel/settings/roomLevels', formState, () => {
        setLoading(false);
        fnGetData();
        fnClear();
      }, (err) => {
      ...
      setLoading(true);
      request.PUT(`hotel/settings/roomLevels/${id}`, formState, () => {
        setLoading(false);
        fnGetData();
        fnClear();
      }, (err) => {
  ...
  const fnDisableDocument = () => {
    ...
    if (formState.id && formState.id > 0) {
      setLoading(true);
      request.PUT(`hotel/settings/roomLevels/${formState.id}`, data, () => {
        fnGetData();
        fnClear();
        setLoading(false);
      }, (err) => {
